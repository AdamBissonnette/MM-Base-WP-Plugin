...
if (!function_exists('createMMFormField')) {
	function createMMFormField($label, $name, $value, $type, $options=null)
	{
		...
		switch ($type)
		{
			...
			break;
			case 'checkbox':
			$field = createMMCheckbox($label, $value, $options);
			break;
			case 'image':
				$field = createMMImageUpload($label, $name, $value, $options);
			break;
			case 'custom':
				$isCustom = true;
				$field = createMMCustomField($label, $name, $value, $options);
			break;
		}
...
function createMMImageUpload($label, $name, $value, $options)
{
	...
	function getMMTaxonomySelectArray($taxonomy, $posts_per_page = -1)
	{
	...
	function createMMCustomField($label, $name, $value, $options)
	{
		echo "test";
	}
}
